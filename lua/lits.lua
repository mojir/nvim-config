...
local function evaluate_lits()
  ...
  local cmd = string.format("lits -f %s 2>&1", vim.fn.shellescape(filepath))
...
local function create_result_popup(result, show_insert_option)
  ...
  if show_insert_option then
    ...
    vim.api.nvim_buf_set_lines(state.result_buf, -1, -1, false, {
      "",
      "Press 'y' to insert, 'n' or <Esc> to cancel"
    })
    vim.bo[state.result_buf].modifiable = false
  end
  ...
  -- Set up keymaps
  local opts = { buffer = state.result_buf, noremap = true, silent = true }
  ...
  if show_insert_option then
    ...
    vim.keymap.set("n", "n", close_result_popup, opts)
  end
  
  -- Create autocmd group for this popup
  local popup_group = vim.api.nvim_create_augroup("LitsResultPopup", { clear = false })
  vim.api.nvim_create_autocmd("WinLeave", {
    group = popup_group,
    buffer = state.result_buf,
    once = true,
    callback = function()
      vim.defer_fn(close_result_popup, 10) -- Small delay to handle quick window switches
    end,
  })
end
...
local function create_program_window(content)
  ...
  -- Alternative keymaps for terminals that don't support C-CR
  vim.keymap.set("n", "<leader>i", evaluate_and_insert, opts)
  vim.keymap.set("i", "<leader>i", evaluate_and_insert, opts)
  
  -- Create autocmd group for this editor
  local editor_group = vim.api.nvim_create_augroup("LitsEditor", { clear = false })
  vim.api.nvim_create_autocmd("WinLeave", {
    group = editor_group,
    buffer = state.editor_buf,
    once = true,
    callback = function()
      vim.defer_fn(close_editor, 10) -- Small delay to handle quick window switches
    end,
  })
  ...
  if use_selection then
    ...
    if existing_content ~= "" then
      local choice = vim.fn.confirm(
        string.format("Replace existing %s content?", state.current_file),
        "&Yes\n&No",
        2
      )
